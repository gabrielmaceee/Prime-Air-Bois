...
Découper par période de temps cf 08/07
...
data <- data[!is.na(data$dossier_reçu),]
data <- data[data$dossier_reçu < "2020-01-01",]
...
utilisation = c("Type.Ancien.appareil", "Usage.ancien.matériel", "Nouveau.matériel", "Type.combustible.nouveau.materiel", "Usage.nouveau.matériel","cout_total_TTC", "économiser", "Gagner en confort/chaleur", "Qualité_air", "type_commune", "majoration", "Territoire", "montant_aide")
data_afmd <- na.omit(data[,utilisation])
```
...
```{r}
for(v in c("cout_total_TTC", "montant_aide")){
  data_afmd[,v] <- scale(data_afmd %>% pull (v))
}
```

# amélioration AFDM : centrer réduire, sélectionner les variables, faire plusieurs ACP selon une variable
```{r}
res.afdm<- FAMD(na.omit(data_afmd), sup.var = c("Territoire", "montant_aide"), graph = FALSE, ncp = 20)
```
...
fviz_famd_ind(res.afdm, habillage =  memb, palette = c("#0073C2FF", "#EFC000FF", "#868686FF","#FC4E07", "green", "orange", "pink"), legend.title = "Cluster",addEllipses = FALSE)
...
set.seed(3)
res.km <- kmeans(res.afdm$ind$coord, centers = 3, nstart = 25)# Créez 2 groupes de variables (centers = 2)
grp <- as.factor(res.km$cluster)
# Colorer les variables par groupes
fviz_famd_ind(res.afdm, habillage = grp, palette = c("#0073C2FF", "#EFC000FF", "#868686FF","#FC4E07", "green", "orange", "pink"), legend.title = "Cluster",addEllipses = FALSE, geom = c("point"))


set.seed(7)
res.km <- kmeans(res.afdm$ind$coord, centers = 2, nstart = 25)# Créez 2 groupes de variables (centers = 2)
grp <- as.factor(res.km$cluster)
# Colorer les variables par groupes
fviz_famd_ind(res.afdm, habillage = grp, palette = c("#0073C2FF", "#EFC000FF", "#868686FF","#FC4E07", "green", "orange", "pink"), legend.title = "Cluster",addEllipses = FALSE, geom = c("point"))
```
...
```{r}
for( v in setdiff(names(data_afmd), c("cout_total_TTC","montant_aide") )){
  print(v)
